...
fn get_plottable_state(
    ...
    let min_rt = apex_rt - 60.0;
    let max_rt = apex_rt + 60.0;
    ...
    // TODO filter here for transitions with super bad intensities. (< 0.1% of base peak)

    let f64_inten_slices: BTreeMap<String, &[f64]> = f64_intensities
        .iter()
...
impl Widget for PlottableChromatogramsState {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let [top_plot, mid_plot, bottom_plot] = Layout::vertical([
        ...
        top_chart.render(top_plot, buf);
        mid_chart.render(mid_plot, buf);
        bottom_chart.render(bottom_plot, buf);
    }
}
